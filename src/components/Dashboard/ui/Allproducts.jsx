...
export default function ProductItems({ name, description, image }){
        const  { id } = useParams();
        const navigate =  useNavigate()

        const { data: allProducts, error, isPending} = useFetch(`http://localhost:5172/allProducts?${id}`);

        const handelClick = () => { 
            if (allProducts) {     
            fetch("http://localhost:5172/allProducts/" + allProducts[0].id, {
                method: 'DELETE'
            }).then(() => {
                navigate('/dashboard');
            })
